Model (
	...
	Req("systemUptimp") has (
		Spec("The system should have an uptime of 99%")),
	Req("gpsEff") has (
		Spec("The system should give a GPS-position correct within a 50 m radius")),
	Req("multiplePlatform") has (
		Spec("Support both Android and IOS")),
	Req("statExtraction") has (
		Spec("Extract statistics from system")),
	Req("viewSavedOnOtherDevice") has (
		Spec("Reach saved desitantions from other devices")),
	Stakeholder("Users"),
	Stakeholder("Staten")
	)

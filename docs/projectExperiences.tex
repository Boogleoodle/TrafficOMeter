...
We have done a project in the course Requirements Engineering. The project has consisted of two parts, one part where we produced a requirements specification for a system and one part where we were clients to another group writing requirements for our system. The larger part was making the system requirements specification. For the actual requirements specification, see the document systemRequirements.pdf. This report describes the process used, techniques and why they were chosen, learning outcomes and a personal statement from each team member on their individual contribution.

	\section{Elicitation Methods/Techniques}
	%Description of the techniques
	%Why we have chosen these methods, what did we want from the different 
Lauesen describes elicitation as "the process of finding and formulating requirements". Elicitation is used for identifying the core values of the project as well as desired features and functions. Identifying requirements seems like a rather simple task but there are several issues that cause a severe threat to the project success. For example that a client cannot express what they need or that they try to give a solution instead of explaining what problem they want solved. These problems are called elicitation barriers. The best way to overcome the elicitation barriers is to use several elicitation techniques and to get input from different stakeholders.

	Some methods that can be used as elicitation techniques are focus group, design workshop or stakeholder analysis. Exactly which elicitation techniques to use depends on the context. In our project we used a few different techniques to approach the problem from different angles. This section explains the techniques we used and how they applied to our project.
...
We used brainstorming mainly in the beginning of the project in order to get a clear idea of the problem we were facing. The process was done in iterations since every session raised new questions that needed to be addressed. The first iterations gave a wider outline of the project and the latter were more detail oriented. Not all sessions were structured and might have been more of group discussions rather than brainstorming.
...
We came to the insight for whom we are really making the system. This led to a meeting with our customers in which we realized that the government should be included, which is something that we hadn't discussed before.
	The following stakeholders were identified:
		...
		After the second release we also found that similar systems, our competitors, might be counted as a stakeholder since if the users use their systems instead our project will not be a success.
...
levels we explain how the different levels of requirements relate to our requirements.  Later on we explain the different techniques we used for specification and what we got out of them.
	...
	\subsection{Levels}
		...
		\subsubsection{Product level}
			The product level requirements define the functions as well as the data too be included in our system.
		\subsubsection{Design level}
			We have not gone into implementation issues such as how to store the user's data but rather focused on what was important for the users, such as which information to display.
...
A data model is an abstract model of what data is needed in a system. It is not necessarily a requirement but in our case we use the model as a requirement.
		...
		\subsubsection{Application}
		We chose the user as a starting point and tried to investigate what kind of data that we would need to be able to give the functionality we thought that the system required. From the entities we came up with from focusing on the user we could come up with several new entities needed and did this recursively until we considered the data for the system to be complete on the level of abstraction we had chosen.
		\subsubsection{Why chosen}
		We used it to realize which kind of data the system would need and how this data would be connected. It also seemed like a good idea to have the data drawn in a model so that the customers could validate it later.
		\subsubsection{Outcome}
	Other than the data model we got a lot of definitions out of putting it together since we needed to discuss what we meant by the entities in the model. These definitions can now be found in a list of definitions in the requirement specification. 

		\subsection{Virtual Windows}
			A virtual window is basically a graphic screen with no actually functions or menus. It is used for visualizing the system's data so that the customer and the developers can relate to it in an easy way.
			\subsubsection{Application}
				We used virtual windows to create a simple data model for the part of the application that handles user interaction. With the virtual windows and the results from the design workshop a suitable screen prototype could be made in a later stage.
			\subsubsection{Why chosen}
				We chose to create some virtual windows because we wanted to know more of the data structure of the user part of the system. Another reason for the selection was that with these virtual windows we could check with the customer that no important data was missing from the model. We think that virtual windows make it easier to visualize how the data will be represented in the system.
			\subsubsection{Outcome}
				We got a clearer view on how the system would interact with the user and what kind of data structure was needed. We also got feedback from the customer on which data that might be missing which made us start over with the creation of the virtual windows.
	...
	\begin{figure}[H]
		...
		\end{figure}
			...
			We found CRUD to be a great way of discovering missing parts in the requirements specification. Even though it can find everything it can check the relationship between tasks and the data model which can reveal missing requirements. 
			
			We also asked our customer to perform a CRUD-check on the system we are writing requirements for. They thought that we were missing task about creating/deleting users which made us realize that we had not been clear about the level of abstraction of our entity user. This was clarified for the final release. 			
			...
			\subsection{Validating with customer checklist}
				We got a checklist from the group that design the requirement specification for our product, Boogle Calendar. We used this checklist to validate their requirements specification, there was contents checks, a CRUD-check and to see that the requirements were written according to the principles from the book (for example unambiguous and correct). When we found that something needed more work we put it down as an issue. After we had gone through the entire checklist we prioritized the found issues for their criticality. This was then sent to the developers so that they had things t improve in their coming release. 
				
				It was a good experience to validate their system since we could get a clearer view of what they had made of the project. We also got the chance to reflect and give constructive feedback so that we could make sure that we got the product we wanted. The benefit of validating the requirement specification became very obvious for us, since in a project in the "real world" validation of the requirement specification would be necessary to make sure that the project is moving in the right direction. We think that in real world projects validation would be done several times during the writing of the requirements specification to see early if it is moving in the right direction.
			...
			\subsection{Goal-Domain tracing}
				...
				To validate our own requirements we decided to do a Goal/Domain Tracing. This means that for each goal we listed which domain requirements that implemented that specific goals. This leads to a completeness check for both the goals and the domain level requirements. We found some requirements that was not possible to fit into a existing requirement and this made us elicit new goals for our stakeholders. When grouping the requirements under a specific goal it gave us a better understanding of what that specific goal should consist of, which lead to some minor adjustment in the wording of the goals to make them clearer. We also found that one of our goals was not implemented by any of the domain requirements, which is a critical problem. Either we were missing requirements or the goal is not correct. As this was one core Goal for one of the stakeholders we decided that the goal was correct and that we were missing a few requirements. After some adding and refactoring of some of the requirements we ended up with a more complete system. 
				
				\subsection{Experiences}
				Validation, both as developers and customers, gave us a lot of good feedback and we felt that it improved the quality of our Requirements Specification. During the validation process we got the chance to break down the project and look at specific parts by them self. This made it easier for us in the requirements team to disregard all the domain knowledge that we had gathered after working with the project for a long time. The validation report provided by our customer also gave us a lot to work with. It was apparent that some requirements that were obvious to us was not so clear when read by another party. So the most interesting results from the checklist was perhaps not the part where the customer gave feedback on things that they deemed needed clarification but the parts where their comment showed us that the had misunderstood or misinterpreted something about the system. The fact that they thought that the system worked in a way (that was not the way we intended it to) exposed the fact that there was a level of ambiguity in the Requirements Specification. This is very dangerous, and if we and the customer do not look the same way on the specification the validation might be incomplete and not be correct according to the customer expectations. Misunderstandings are never a good thing. In our case we tried clarify all the sections of the Requirements Specification where we found any indication of ambiguity. Mainly by rewriting the text and giving more examples. 
	...
	%
		Release planning is a vital part for a project's success. Basically release planning is about determining which requirements and features that should be included in which release. Determining this is NP-hard and often have more than one solution.
		
		\subsection{Why chosen}
		There are different methods of performing release planning, either by hand or using some sort of tool. However if the project is complex and contains many requirements it is almost impossible to do it by hand. We decided to try doing it by hand first and quickly realised the task was too complex. Below we describe the different techniques.

			\subsubsection{Release planning by hand}
			\label{sec:release_planning}
			As our project does not contain a lot of dependencies between features it was not that hard to generate an adequate solution. However, if the number of dependencies increase, planning by hand would be much more difficult. As the dependencies increase in number it becomes more obvious that release planning actually is a NP-hard problem.

			\subsubsection{Release planning with software}
			Release planning with a release planning software have several advantages. Firstly, release planning is seldom done as an isolated event and having a software that is integrated with the one for handling requirements will greatly ease the planning. Secondly, release planning is always done several times over the project and by using a software one can update the release planning if new requirements are added, requirements removed or if any problems occur during a the implementation of a release. In general, using a software to perform the release planning it becomes more flexible and powerful, even if it might sometimes be a hurdle to overcome in the beginning. For our release planning we chose to use reqT as we already used this software to manage our requirements.

			\subsubsection{Our experiences with reqT} %TA BORT HELA SUBSUBSECTION?!
				All requirements that should be included in the release planning has to be specified, given a cost to implement from each development team, given a benefit for each stakeholder, and constraints such as feature and requirement precedences much be entered. This is not a problem if done continuously throughout the project.

		\subsection{Outcome}
		
		Doing the release planning gave a clearer view of what the prioritization of the different requirements really meant. However we encountered a few problems along the way. Since we did prioritize the requirements with different methods for different stakeholders, it became difficult to make a release planning covering all stakeholders. We solved this by removing the benefits from the stakeholder that did not use the 100-dollar method. This could result in a misleading release plan, since not all stakeholders' benefits will be taken into account. Another problem that we encountered that could result in a misleading release plan is that some of the stakeholders might not really know what requirements they deem important, or may not take into account what other parts need at all. It may also be that their knowledge of the system is not enough to give a proper prioritization. This could be solved by having a wide selection of stakeholders with different benefits from the system, leading to a more balanced release plan because they are all pulling in their own direction. 
%FRÅN HÄR, ÄNDRA? Jag ser definitivt benefits över att planera mer än en release framåt.		
		In our experience it is hard to maximize more than one release at the time in reqT, which lead to us only planning one proper release and one release containing all the other requirements that did not fit in the first. We do not see this as a problem for this project since it is not necessary for us to plan more than one release ahead, although it would be very convenient to cover all releases in one planning session. Planning all releases in one session is in most cases not possible, since release planning is a continuous work and there might emerge new requirements or other problems along the way.				
...
If we were to continue with the project it would be beneficial to elicit more requirements. In general, our quality requirements could need more work. In order to improve and find new quality requirements we would try new elicitation techniques. Some of the techniques we would want to use are:
	...
	Below we discuss reflections about the process and how the project has went. Reflections on specific methods can be found under respective section. The reflections here are more general, concerning project process etcetera.
...
For the third release we worked a lot with the comments we got from our project supervisor and our customers. We also validated release two that we got from the group that we are customers to, Boogle Calendar. We also did a release plan for our product. Release planning turned out to be quite hard as well, see section \ref{sec:release_planning} for further detatils.

Validating the Boogle Calendar requirement specification was not easy. It was hard to identify all the fields in the validation checklist that we got and to find out if the specification lived up to them. We first did the validation individually and during our next work session some of us sat together and merged our different opinions into one final validation report. This way of doing the validation incorporated every member in the group in some way. Furthermore, it gave us the opportunity to both work on the validation and our own project in the same time. Also, because the sub-part of the group that finalized the report could discuss a lot, the result was quite good. The drawback is that some of us had to do the validation multiple times and lost some time when they could have worked with our project instead. 

The customer validation of Traffic-o-meter gave us a different point of view of our requirements specification which lead to that we understood that all parts were not as clear as they could be. For example the customer did a CRUD-matrix of all the entities in the data model/definitions and found some missing actions. They specifically said that they did not find any tasks for creating and updating the User. But to us this was not a problem as we saw the User as the actual person using the application, and not a user account that needs to be created and updated (we decided to leave this solution to the developers as a user account might not be the most efficient solution). This was obviously not as clear as it should have been and we therefore decided to make an extra note explaining our thoughts on this. The goal of the added note was both to explain the User a bit more in detail but also to make the reader of the document to understand our reasoning when taking the decision. 

Over all the project has been a learning experience and we are satisfied with the result. We feel that some parts of our project could need some more work, which are quality requirements and release planning. These are the parts we would focus on if we had a future release.
...
In this paragraph each group member briefly explains its individual contributions to the project result.
	...
	\subsection{Martin}
		...
		\subsubsection{R3}
			For the final release cycle I did the presentation with Philip as well as working with the project experience. 

...
Brainstorming is a good method to use in the beginning of a project to get started. One reason for this is that brainstorming is a good way to get a grip of the current situation in a project and get an idea of the problem at hand. It is an easy to use method that does not require a lot of preparation or material. It is quite fast and the result is direct. Just a couple of people from the project group can brainstorm for 10 minutes on a topic and then share with the rest of the group what they came up with. 
 ...
 We combined brainstorming and stakeholder analysis to answer the questions above. This was done mostly within the group but we also discussed with our clients and with our supervisor.
		...
		\subsubsection{Outcome}
The outcome of using stakeholder analysis was that we came to insight for whom we are really making the system.  This led to a meeting with our customers in which we realized that the government should be included, which is something that we hadn't discussed before. There was also some entities that are affected by the product but not considered as stakeholders, such as rivalling companies and services. The different entities are hard to tell apart before making a stakeholder analysis but are very important for the outcome of other parts of the project, such as the release planning. Having determined the stakeholders made some parts of the elicitation and specification easier, as we knew more about who were affected by the product. This made creation of goal requirements easier as we could tie them to a specific stakeholder.
...
A data model is an abstract model of what data is needed in a system. It is not necessarily a requirement but in our case we use the model as a requirement. Data model is an efficient and unambiguous way of visualizing the data the system requires. Data models are similar in structure to E/R-diagrams which makes them more suitable for the developers.
	...
	We got a lot of definitions out of putting it together since we needed to discuss what we meant by the entities in the model (coming in a data dictionary in release 2).
		...
		\subsection{Virtual Windows}
			...
			\subsubsection{Application}
				We used Virtual Widows to create a simple data model for the Application and the corresponding Widget. Together with the Design workshop a suitable screen prototype could be made.
			\subsubsection{Why chosen}
				We needed to get more knowledge about the data structure of the user interface of the System. Since Virtual Windows focuses on what kind of data is needed in specific interfaces of the system, it was a natural choice.
			\subsubsection{Outcome}
				We got clearer view on how the system would interact with the customer and what kind of data structure was needed. It really gave us a good start on what kind of data was needed and where the data was need. Together wit the Design Workshop in our "project backpack", it gave us a good start for the designing of the screens.

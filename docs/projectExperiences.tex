...
A data model is an abstract model of what data is needed in a system. It is not necessarily a requirement but in our case we use the model as a requirement.
	...
	%
		Release planning is (just as many other aspects in this document) vital for a project's success. Basically release planning is about determining which requirements and features that should be included in which release. Determining this is NP-hard and often have more than one solution. For reference, when we performed the release planning for our project, we managed to find approximately 450000 different solutions in 2 minutes (this was of course not done by hand).
		\subsection{Why chosen}
		There are different methods of performing release planning, either by hand or use some sort of tool. However if the project is complex and contain many requirements it is almost impossible to do it by hand. We decided to try doing it by hand first and quickly realised the task was too complex. Below we have described the different techniques.
